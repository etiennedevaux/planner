...
For leisure users the drone rules are complex, and require extensive reading to understand and interpret.  This leads to most leisure drone users taking a 'common-sense' approach, which can lead to problems and potentially serious problems including fatalities.
...
This data is feely available from the (*Open Postcode Geo*)[https://www.getthedata.com/open-postcode-geo-api] freely available with attribution[https://www.ons.gov.uk/methodology/geography/licences]

A proof of concept page was created to ensure tha the technology could return decimal latitude and longitude suitable for use in both the Google Maps and Altitude Angel API and is stored within the project space on GitHub.

THe POC file is called PostCodeLookup [https://etiennedevaux.github.io/planner/projectdocuments/poccode/PostCodeLookup.html] and provides the latitude and longitude in the correct format for a single postcode (CV31 3PE) as the code to input the desired postcode was not required to prove the concept.
...
A proof of concept page was created to ensure tha the technology could return decimal latitude and longitude suitable for use in both the and is stored within the project space on GitHub.

THe POC file is called AltititudeAngelAPI [https://etiennedevaux.github.io/planner/projectdocuments/poccode/AltititudeAngelAPI.html] and provides a surface level air traffic map for a given latitude and longitude.

The POC file was provided by Altitude Angel and was taken from the sample source code at [https://developers.altitudeangel.com/] NB Requires registration and login.
